/**
 * Clase PlantillaFormularioResource.java 10 nov. 2025 Copyright 2025 Servicio de Rentas Internas.
...
import ec.gob.sri.api.catalogo.application.dto.ListarPlantillaFormulariosResponse;
...
public class PlantillaFormularioResource {
    ...
    public Uni<Response> listar(
        ...
        @Parameter(description = "Texto de búsqueda (busca en código)") @QueryParam("codigo") String codigo,
    ...
    @POST
    @Path("/{codigoPlantillaFormulario}/duplicar")
    @Operation(summary = "Duplicar un formulario", description = "Crea una copia de un formulario existente con el sufijo 'Duplicado' en nombre y descripción")
    @APIResponse(responseCode = "201", description = "Formulario duplicado exitosamente", content = @Content(mediaType = MediaType.APPLICATION_JSON, schema = @Schema(implementation = GuardarPlantillaFormularioResponse.class)))
    @APIResponse(responseCode = "404", description = "Formulario no encontrado", content = @Content(mediaType = MediaType.APPLICATION_JSON, examples = {
        @ExampleObject(name = "Ejemplo 404", value = """
            { "codigo": "ERR-404", "mensaje": "No se encontró el formulario con ID: 1" }
            """)
    }))
    @APIResponse(responseCode = "500", description = "Error interno del servidor", content = @Content(mediaType = MediaType.APPLICATION_JSON, examples = {
        @ExampleObject(name = "Ejemplo 500", value = ErrorMessages.EXAMPLE_ERROR_500)
    ...
    public Uni<Response> duplicar(
        @Parameter(description = "Código de plantilla formulario", required = true) @PathParam("codigoPlantillaFormulario") Long codigoPlantillaFormulario) {
        return service.duplicar(codigoPlantillaFormulario)
            .onItem()
            .transform(result -> Response.status(Response.Status.CREATED).entity(result).build())
            .onFailure(IllegalArgumentException.class)
            .recoverWithItem(err -> Response.status(Response.Status.NOT_FOUND)
