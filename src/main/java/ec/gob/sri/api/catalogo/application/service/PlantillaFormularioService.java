...
public class PlantillaFormularioService {
    ...
    public Uni<GuardarPlantillaFormularioResponse> guardar(
        ...
        return repository.buscarPorCodigoYVersion(request.codigo, request.version)
            .onItem().transformToUni(existente -> {
                ...
                plantilla.setElementosXml(convertirElementosAJson(request.elementosXml));
    ...
    public Uni<GuardarPlantillaFormularioResponse> actualizar(
        ...
        return repository.buscarPorId(codigoPlantillaFormulario)
            .onItem().transformToUni(
                existente -> procesarActualizacion(request, codigoPlantillaFormulario, existente));
    }
    ...
    private Uni<GuardarPlantillaFormularioResponse> validarYActualizar(
        ...
        return repository.buscarPorCodigoYVersion(codigoABuscar, versionABuscar)
            .onItem()
            .transformToUni(
                otro -> validarDuplicadoYActualizar(request, existente, otro, codigoABuscar,
                    versionABuscar));
    }
    ...
    private Uni<GuardarPlantillaFormularioResponse> actualizarPlantilla(
        ...
        if (request.elementosXml != null) {
            existente.setElementosXml(convertirElementosAJson(request.elementosXml));
        }
     ...
     * <p>
     * Responsabilidades: - Orquestar el flujo de negocio - Normalizar parámetros de entrada -
     * Transformar respuesta a DTOs de salida - NO contiene lógica de infraestructura ni
    ...
    public Uni<PaginadoResponse<GuardarPlantillaFormularioResponse>> listar(
        ...
        var parametrosPaginacion = normalizarParametrosPaginacion(request);

        // Delegación al repositorio (responsabilidad: acceso a datos)
        return repository.listar(parametrosPaginacion.page(), parametrosPaginacion.sort(),
                request.codigo,
                request.buscar)
            .map(resultado -> {
                // Mapeo a DTOs de salida (responsabilidad: transformación a transfer objects)
                List<GuardarPlantillaFormularioResponse> contenido =
                    mapper.toResponseListConElementosParsados(resultado.contenido, objectMapper);
    ...
    /**
     * Normaliza parámetros de paginación desde la solicitud del cliente Responsabilidad de capa de
     * aplicación: validar y transformar inputs
     *
     * @param request Solicitud del cliente con parámetros potencialmente inválidos o nulos
     * @return Record con Page y Sort normalizados
     */
    private ParametrosPaginacion normalizarParametrosPaginacion(
        ListarPlantillaFormulariosRequest request) {
     ...
     */
    public Uni<GuardarPlantillaFormularioResponse> duplicar(Long codigoPlantillaFormulario) {
        return repository.buscarPorId(codigoPlantillaFormulario)
            .onItem().transformToUni(existente -> {
                if (existente == null) {
                    return Uni.createFrom().failure(
                ...
                // Crear el request con los datos del existente
                GuardarPlantillaFormularioRequest request = new GuardarPlantillaFormularioRequest();

                // Generar código único agregando timestamp
                String timestamp = String.valueOf(System.currentTimeMillis());
                request.codigo = "-DUPLICADO-" +
                    existente.getCodigo() + "-" + timestamp.substring(
                    timestamp.length() - 6);
                request.nombre = "DUPLICADO " + existente.getNombre();
                request.descripcion = "DUPLICADO " + existente.getDescripcion();
                request.version = existente.getVersion();
                request.estado = EstadoPlantilla.EC.getCodigo(); // Por defecto EN CONSTRUCCIÓN

                // Parsear elementosJson de String a Object para el request
                try {
                    if (existente.getElementosJson() != null) {
                        request.elementosJson = objectMapper.readValue(
                            existente.getElementosJson(),
                            Object.class);
                    }
                } catch (JsonProcessingException e) {
                    return Uni.createFrom().failure(
                        new IllegalArgumentException(
                            "Error al parsear elementos JSON: " + e.getMessage(), e));
                }

                request.elementosXml = existente.getElementosXml();

                // Usar el mapper para crear el dominio desde el request
                PlantillaFormulario duplicado = mapper.toDomainFromRequest(request);

                // Validar y establecer el estado
                duplicado.setEstado(validarYConvertirEstado(request.estado));

                // Convertir elementos Object a JSON String
                duplicado.setElementosJson(convertirElementosAJson(request.elementosJson));
                duplicado.setElementosXml(convertirElementosAJson(request.elementosXml));

                return repository.guardar(duplicado)
                    .map(mapper::toResponse);
            });
    }
    ...
    public Uni<GuardarPlantillaFormularioResponse> cambiarEstado(Long codigoPlantillaFormulario,
        ...
        return repository.buscarPorId(codigoPlantillaFormulario)
            .onItem().transformToUni(
                existente -> procesarCambioEstado(codigoPlantillaFormulario, request, existente));
    }
    ...
    private String convertirElementosAJson(Object elementos) {
        ...
        try {
            ...
            throw new IllegalArgumentException(
                "Error al convertir páginas a JSON: " + e.getMessage(), e);
        }
    }
    ...
    /**
     * Record para encapsular parámetros normalizados de paginación Responsabilidad: transportar
     * parámetros validados a través de las capas
     */
